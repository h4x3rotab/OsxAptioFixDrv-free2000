...
EFI_STATUS FreeLowMemory(UINT64 KernelTopAddress)
{
    EFI_STATUS				Status;
    UINTN					MemoryMapSize;
    EFI_MEMORY_DESCRIPTOR	*MemoryMap;
    ...
    if (EFI_ERROR(Status)) {
        Print(L"GetMemroyMap error\n");
        return Status;
    }
    
    Status = EFI_NOT_FOUND;
    
    Print(L"MEMMAP: Size=%d, Addr=%p, DescSize=%d, DescVersion: 0x%x\n", MemoryMapSize, MemoryMap, DescriptorSize, DescriptorVersion);
    Print(L"Type       Start            End       VStart               # Pages          Attributes\n");
    ...
    for (Desc = MemoryMap ; Desc < MemoryMapEnd; Desc = NEXT_MEMORY_DESCRIPTOR(Desc, DescriptorSize)) {
        ...
        if (Desc->Type == EfiConventionalMemory												// free mem
            ...
            || Desc->PhysicalStart >= KernelTopAddress									    // above critical space
            )
        {
            continue;
        }
